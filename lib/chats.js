...
router.get('/:id/user1', getChat, async (request, response) => {
  response.send(response.chat.user1);
});

//Get user2 of one chat
router.get('/:id/user2', getChat, async (request, response) => {
...
router.get('/:name1/:name2', async (request, response) => {
  try {
    const chat = await Chat.find({
      $or: [
        { $and: [{ user1: request.params.name1 }, { user2: request.params.name2 }] },
        { $and: [{ user1: request.params.name2 }, { user2: request.params.name1 }] }
      ]
    });
